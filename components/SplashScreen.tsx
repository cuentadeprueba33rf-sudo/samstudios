...
export const SplashScreen: React.FC<SplashScreenProps> = ({ onFinish }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-surface flex items-center justify-center overflow-hidden perspective-[1000px]">
      ...
      {/* Ambient Deep Glow */}
      <motion.div 
        className="absolute inset-0 bg-[radial-gradient(circle_at_center,_rgba(182,160,255,0.15)_0%,_transparent_60%)]"
        initial={{ opacity: 0 }}
      ...
      <AnimatePresence>
        {stage < 3 && (
          ...
          >
            ...
            {stage === 1 && (
              <motion.div
                className="absolute h-[2px] bg-primary shadow-[0_0_20px_2px_#b6a0ff,0_0_40px_4px_#00affe]"
            ...
            {/* The Text */}
            <div className="flex space-x-1 sm:space-x-4 overflow-visible px-4 relative">
              {letters.map((letter, i) => (
                <motion.span
                  key={i}
                  className="text-6xl sm:text-8xl md:text-[10rem] font-headline font-black tracking-[0.2em] relative"
                  initial={{ 
                    opacity: 0, 
                    y: 20, 
                    scale: 1.1,
                ...
                >
                  {/* Base Text */}
                  <span className="text-transparent bg-clip-text bg-gradient-to-b from-on-surface via-primary to-secondary relative z-10">
                    {letter === " " ? "\u00A0" : letter}
                  </span>
                  
                  {/* Glowing Outline / Shadow */}
                  <motion.span 
                    className="absolute inset-0 text-primary blur-[24px] z-0"
                    initial={{ opacity: 0 }}
              ...
              {stage >= 2 && (
                <motion.div
                  className="absolute top-0 bottom-0 w-32 bg-gradient-to-r from-transparent via-white to-transparent opacity-20 blur-[10px] skew-x-[-25deg] z-20 mix-blend-overlay"
                  initial={{ left: "-30%" }}
                  animate={{ left: "130%" }}
            ...
            {/* Subtitle */}
            <motion.div
              className="absolute -bottom-16 sm:-bottom-20 text-on-surface-variant text-[10px] sm:text-[12px] font-label font-bold tracking-[1.5em] uppercase"
